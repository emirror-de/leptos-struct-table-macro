...
fn get_renderer_for_field(name: &syn::Ident, field: &TableDataField, index: usize) -> TokenStream2 {
    ...
    let format_props = get_format_props_for_field(name, field);
            ...
            Ok(inner_type_ident) => {
                ...
                quote! {
                    <Show when=move || { item.#getter.is_some() }
                        fallback=move || view!{<DefaultTableCellRenderer value=#none_value.to_string() #class_prop #index_prop #editable_prop />}
                    >
                        #inner_renderer
                    </Show>
                }
...
fn get_format_props_for_field(name: &syn::Ident, field: &TableDataField) -> TokenStream2 {
...
fn get_selection_logic(
    ...
    match selection_mode {
        ...
        SelectionMode::Single => (
            ...
            quote! {create_selector(move || selected_key.get())},
        ),
    ...
    fn to_tokens(&self, tokens: &mut TokenStream2) {
            ...
            titles.push(quote! {
                <#head_renderer
                    ...
                    sort_priority=Signal::derive(move || {
                        ...
                        sorting.get().iter().position(|(field, _)| *field == #column_name_enum::#column_name_variant)
                    })
        ...
        tokens.extend(quote! {
                ...
                // FH: Fn(FieldValue) + 'static,
            {
                ...
                let action_set_row = create_action(move |(key, row): &(#key_type, #ident)| {
                ...
                let local_items_state = create_rw_signal(None::<Vec<#ident>>);
                ...
                let memo_update_local_items_state = move || {
                    local_items_state.set(fetched_items.read());
                };

                let broadcast_local_items_state = create_memo(move |_| {
                ...
                let memo_items = create_memo(move |_| {
